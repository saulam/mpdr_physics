...
class AELightningModel(pl.LightningModule):
    ...
    def on_train_epoch_start(self):
        ...
        train_loader.dataset.datasets.dataset.datasets[0].set_training_mode(True)
        train_loader.dataset.datasets.dataset.datasets[1].set_training_mode(True)


    def on_validation_epoch_start(self):
        """Hook to be called at the start of each validation epoch."""
        val_loader = self.trainer.val_dataloaders[0]
        val_loader.dataset.dataset.datasets[0].set_training_mode(False)
        val_loader.dataset.dataset.datasets[1].set_training_mode(False)
...
class NAELightningModel(pl.LightningModule):
    ...
    def validation_step(self, batch1, batch2, batch_idx):
        """Validation step for a batch of data.""" 
        return
    ...
    def validation_epoch_end(self, outputs):
        ...
        ood_val_loader = self.trainer.val_dataloaders[1]

        in_pred = self.predict(indist_val_loader)
        ood_pred = self.predict(ood_val_loader)
        auc_val = roc_btw_arr(ood_pred, in_pred)

        # Log losses
        self.log(f"nae/auc_val", auc_val, prog_bar=True, sync_dist=True)

        indist_val_loader.dataset.dataset.datasets[0].set_training_mode(True)
        indist_val_loader.dataset.dataset.datasets[1].set_training_mode(True)
