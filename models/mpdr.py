...
class AE(nn.Module):
    ...
    def train_step(self, x, optimizer, manual_backward=None, scheduler=None, clip_grad=None, x_neg=None, mode=None, **kwargs):
        ...
        if manual_backward is not None:
...
class MPDR_Single(nn.Module):
    ...
    def train_step_ae(self, x, optimizer, manual_backward=None, scheduler=None, clip_grad=None):
        """train all autoencoders in the list"""
        return self.ae.train_step(x, optimizer=optimizer, manual_backward=manual_backward, scheduler=scheduler, clip_grad=clip_grad)
    ...
    def train_step(self, x, optimizer, manual_backward=None, scheduler=None, clip_grad=None, y=None, mode="off"):
        ...
        if self.work_on_manifold:
            with torch.no_grad():
                x = self.decode(self.encode(x))
        d_train = self.train_step_off(x, optimizer=optimizer, manual_backward=manual_backward, scheduler=scheduler, y=y)
        # else:
        #     raise ValueError("mode should be off: {}".format(mode))
        return d_train

    def train_step_off(self, x, optimizer, manual_backward=None, scheduler=None, y=None):
        ...
        d_loss = self.update_model_off(x.detach(), x_neg.detach(), optimizer, manual_backward=manual_backward, scheduler=scheduler, y=y)

        return {**d_train, **d_loss}

    def update_model_off(self, x, x_neg, optimizer, manual_backward=None, scheduler=None, y=None):
